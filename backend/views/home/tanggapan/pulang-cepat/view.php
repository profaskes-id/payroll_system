...
<?php if (empty($model) || empty($model['id_izin_pulang_cepat'])) : ?>
...
<?php else : ?>
    ...
    <div class="container relative z-50 p-0 mx-auto md:p-6">
        <div class="flex items-center justify-between">
            <h1 class="mb-4 text-2xl font-bold">Detail Pengajuan Pulang Cepat</h1>
            <p><?= Html::a('Kembali', ['/tanggapan/pulang-cepat'], ['class' => 'costume-btn']) ?></p>
        </div>

        <div class="flex items-center justify-between my-5">
            <div class="flex justify-start space-x-4">
                <?= Html::a('Tanggapi', ['pulang-cepat-update', 'id' => $model->id_izin_pulang_cepat], ['class' => 'tw-add bg-blue-500 px-6 relative']) ?>

                <?= Html::button('Delete', [
                    'class' => 'btn-delete tw-add bg-red-500 px-5 text-white rounded',
                    'data-url' => Url::to(['pulang-cepat-delete', 'id' => $model['id_izin_pulang_cepat']])
                ]) ?>
            </div>
        </div>
        ...
        <div class="overflow-hidden bg-white rounded-lg shadow-md">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <tr>
                        <td class="px-6 py-4 text-sm font-medium text-gray-900">Karyawan</td>
                        <td class="px-6 py-4 text-sm text-gray-700"><?= Html::encode($model->karyawan->nama ?? '-') ?></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 text-sm text-gray-700">
                            ...
                            echo match ($model->status) {
                                0 => '<span class="text-yellow-500">Pending</span>',
                                1 => '<span class="text-green-500">Disetujui</span>',
                                2 => '<span class="text-red-500">Ditolak</span>',
                                default => '<span class="text-rose-500">Status tidak valid</span>',
                            };
                            ?>
                        </td>
                    </tr>
                    <tr>
                        <td class="px-6 py-4 text-sm font-medium text-gray-900">Disetujui Oleh</td>
                        <td class="px-6 py-4 text-sm text-gray-700">
                            <?php
                            if ($model->user) {
                                echo Html::encode($model->user->profile->full_name ? $model->user->profile->full_name : $model->user->username ?? 'Belum disetujui');
                            }
                            ?>
                        </td>
                    </tr>
                    <tr>
                        <td class="px-6 py-4 text-sm font-medium text-gray-900">Tanggal Disetujui</td>
                        <td class="px-6 py-4 text-sm text-gray-700">
                            <?= $model->disetujui_pada ? Yii::$app->formatter->asDatetime($model->disetujui_pada) : 'Belum ditanggapi' ?>
                        </td>
                    </tr>
                    <tr>
                        <td class="px-6 py-4 text-sm font-medium text-gray-900">Catatan Admin</td>
                        <td class="px-6 py-4 text-sm text-gray-700">
                            <?= Html::encode($model->catatan_admin ?? 'Tidak ada catatan') ?>
                        </td>
                    </tr>
                </tbody>
            </table>
