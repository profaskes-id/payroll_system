...
<h1 class="p-5 text-2xl font-bold">Pengajuan Dinas</h1>
...
<div class="relative z-50 px-5 overflow-x-auto">
    <table class="w-full text-sm text-left text-gray-500 rtl:text-right dark:text-gray-400" id="table_id" class="display">
        ...
        <tbody>
            ...
            <?php
            foreach (array_reverse($pengajuanDinasList) as $item) {
                $statusColor = '';
                $statusText = '';
                switch ($item['status']) {
                    case 0:
                ...
                <tr class="bg-white border-b border-gray-200 dark:bg-gray-800 dark:border-gray-700">
                    <td class="px-6">
                        <p class="flex items-center justify-start">
                            <a href="<?= Yii::$app->urlManager->createUrl(['/tanggapan/dinas-view', 'id_pengajuan_dinas' => $item['id_pengajuan_dinas']]) ?>" class="relative flex items-center  justify-center p-1.5 bg-[#488aec] rounded-md">
                    ...
                    <td class="px-6 py-1 text-xs">
                        <?= $item->keterangan_perjalanan
                        ?>
                    </td>
                    <td class="px-6 py-1 text-xs">


                        <?php echo "<p class='text-xs'> Rp "  . number_format($item->estimasi_biaya, 0, ',', '.') . "</p>";
