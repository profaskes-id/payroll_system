    ...
    <table class="w-full text-sm text-left text-gray-500 rtl:text-right dark:text-gray-400" id="table_id">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                ...
                <th scope="col" class="px-6 py-3">Alasan Pengajuan</th>
                <th scope="col" class="px-6 py-3">Status</th>
            </tr>
        </thead>
        <tbody>
            <?php foreach (array_reverse($pengajuanAbsensiList) as $item): 
                $statusClass = '';
                $statusText = '';
                switch ($item['status']) {
                    case 0:
            ...
            ?>
                <tr class="bg-white border-b border-gray-200 dark:bg-gray-800 dark:border-gray-700">
                    <td class="px-6 py-4">
                        <a href="<?= Yii::$app->urlManager->createUrl(['/tanggapan/absensi-view', 'id' => $item['id']]) ?>" 
                    ...
                    <td class="px-6 py-4"><?= $item->tanggal_absen ?></td>
                    <td class="px-6 py-4"><?= $item->jam_masuk ?? '-' ?></td>
                    <td class="px-6 py-4"><?= $item->jam_keluar ?? '-' ?></td>
                    <td class="px-6 py-4"><?= $item->alasan_pengajuan ?? '-' ?></td>
    ...
    $(document).ready(function() {
        $('#table_id').DataTable({
            ...
            columnDefs: [
                ...
                { responsivePriority: 3, targets: 5 }, // Alasan Pengajuan
