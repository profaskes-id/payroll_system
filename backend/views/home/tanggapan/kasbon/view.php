...
<?php if (empty($model) || empty($model['id_pengajuan_kasbon'])) : ?>
...
<?php else : ?>
    ...
    <div class="container relative z-50 p-0 mx-auto md:p-6">
        ...
        <div class="flex items-center justify-between">
            <h1 class="mb-4 text-2xl font-bold">Detail Pengajuan Kasbon</h1>
            <p>
                <?= Html::a('Back', ['/tanggapan/kasbon'], ['class' => 'costume-btn']) ?>
            </p>
        </div>

        <div class="flex items-center justify-between my-5">
            <div class="flex justify-start space-x-4">
                <?= Html::a('Tanggapi', ['kasbon-update', 'id' => $model['id_pengajuan_kasbon']], ['class' => 'tw-add bg-blue-500 px-6 relative text-white rounded']) ?>
                <?= Html::button('Delete', [
                    'class' => 'btn-delete tw-add bg-red-500 px-5 text-white rounded',
                    'data-url' => Url::to(['kasbon-delete', 'id_pengajuan_kasbon' => $model['id_pengajuan_kasbon']])
                ]) ?>
            </div>
        </div>
        ...
        <div class="overflow-hidden bg-white rounded-lg shadow-md">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        <td class="px-6 py-4 text-sm font-medium text-gray-900 whitespace-nowrap">Gaji Pokok</td>
                        <td class="px-6 py-4 text-sm text-gray-700 whitespace-nowrap"><?= "Rp " . number_format($model['gaji_pokok'], 0, ',', '.') ?></td>
                    </tr>

                    <tr>
                        <td class="px-6 py-4 text-sm font-medium text-gray-900 whitespace-nowrap">Jumlah Kasbon</td>
                        <td class="px-6 py-4 text-sm text-gray-700 whitespace-nowrap"><?= "Rp " . number_format($model['jumlah_kasbon'], 0, ',', '.') ?></td>
                    </tr>
                    ...
                    <tr>
                        <td class="px-6 py-4 text-sm font-medium text-gray-900 whitespace-nowrap">Tanggal Pencairan</td>
                        <td class="px-6 py-4 text-sm text-gray-700 whitespace-nowrap"><?= $model['tanggal_pencairan'] ? $tanggalFormater->getIndonesiaFormatTanggal($model['tanggal_pencairan']) : 'Belum dicairkan' ?></td>
                    </tr>

                    <tr>
                        <td class="px-6 py-4 text-sm font-medium text-gray-900 whitespace-nowrap">Lama Cicilan</td>
                        <td class="px-6 py-4 text-sm text-gray-700 whitespace-nowrap"><?= Html::encode($model['lama_cicilan'] . ' bulan') ?></td>
                    </tr>

                    <tr>
                        <td class="px-6 py-4 text-sm font-medium text-gray-900 whitespace-nowrap">Angsuran Per Bulan</td>
                        <td class="px-6 py-4 text-sm text-gray-700 whitespace-nowrap"><?= "Rp " . number_format($model['angsuran_perbulan'], 0, ',', '.') ?></td>
                    </tr>

                    <tr>
                        <td class="px-6 py-4 text-sm font-medium text-gray-900 whitespace-nowrap">Tanggal Mulai Potong</td>
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 text-sm text-gray-700 whitespace-nowrap"><?= Html::encode($model['keterangan'] ?? 'Tidak ada keterangan') ?></td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 text-sm text-gray-700 whitespace-nowrap">
                            <?php
                            if ($model['status'] == 0) {
                                echo '<span class="text-yellow-500">Menunggu Persetujuan</span>';
                            } elseif ($model['status'] == 1) {
                                echo '<span class="text-green-500">Disetujui</span>';
                            } elseif ($model['status'] == 2) {
                                echo '<span class="text-red-500">Ditolak</span>';
                            } else {
                                echo '<span class="text-gray-500">Tidak Diketahui</span>';
                            }
                            ?>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 text-sm text-gray-700 whitespace-nowrap"><?= $model['tanggal_disetujui'] ? $tanggalFormater->getIndonesiaFormatTanggal($model['tanggal_disetujui']) : 'Belum disetujui' ?></td>
                    </tr>

                    <tr>
                        <td class="px-6 py-4 text-sm font-medium text-gray-900 whitespace-nowrap">Disetujui Oleh</td>
                        <td class="px-6 py-4 text-sm text-gray-700 whitespace-nowrap"><?= Html::encode($model['user']['profile']['full_name'] ?? $model['user']['username'] ?? 'Belum disetujui') ?></td>
                    </tr>
