...
<h1 class="p-5 text-2xl font-bold">Pengajuan Kasbon</h1>
    ...
    <table class="w-full text-sm text-left text-gray-500 rtl:text-right dark:text-gray-400" id="table_id">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                ...
                <th scope="col" class="px-6 py-1">Nama</th>
                <th scope="col" class="px-6 py-1">Jumlah Kasbon</th>
                <th scope="col" class="px-6 py-1">Lama Cicilan</th>
                <th scope="col" class="px-6 py-1">Angsuran / Bulan</th>
                <th scope="col" class="px-6 py-1">Tanggal Pengajuan</th>
                <th scope="col" class="px-6 py-1">Tanggal Mulai Potong</th>
                <th scope="col" class="px-6 py-1">Status</th>
            </tr>
        </thead>
        <tbody>
            <?php
            foreach (array_reverse($pengajuanKasbonList) as $item) {
                $statusColor = '';
                $statusText = '';
                switch ($item['status']) {
                    case 0:
                ...
                <tr class="bg-white border-b border-gray-200 dark:bg-gray-800 dark:border-gray-700">
                    <td class="px-6">
                        <p class="flex items-center justify-start">
                            <a href="<?= Yii::$app->urlManager->createUrl(['/tanggapan/kasbon-view', 'id_pengajuan_kasbon' => $item['id_pengajuan_kasbon']]) ?>" class="relative flex items-center justify-center p-1.5 bg-[#488aec] rounded-md">
                    ...
                    <td class="px-6 py-1 text-xs">
                        <?= $tanggalFormater->getIndonesiaFormatTanggal($item->tanggal_pengajuan) ?>
                    </td>
                    <td class="px-6 py-1 text-xs">
                        <?= $item->tanggal_mulai_potong ? $tanggalFormater->getIndonesiaFormatTanggal($item->tanggal_mulai_potong) : '-' ?>
                    </td>
